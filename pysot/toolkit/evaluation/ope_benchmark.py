    ...
    def eval_success(self, eval_trackers=None):
        ...
        for tracker_name in eval_trackers:
            ...
            for video in self.dataset:
                ...
                success_path = os.path.join('/home/dc/luohongfu/pysot/OPEBenchmark/success', video.name)
                if not os.path.isdir(success_path):
                    os.makedirs(success_path)
                success_result_path = os.path.join(success_path,'{}.txt'.format(tracker_name+video.name))
                with open(success_result_path, 'w') as f:
                    for x in success_ret_[video.name]:
                        f.write(str(x) + '\n')
        ...
        for tracker_name in eval_trackers:
            ...
            for video in self.dataset:
                ...
                precision_ret_[video.name] = success_error(gt_center, tracker_center,
                        thresholds, n_frame)
                precision_path = os.path.join('/home/dc/luohongfu/pysot/OPEBenchmark/precision', video.name)
                if not os.path.isdir(precision_path):
                    os.makedirs(precision_path)
                precision_result_path = os.path.join(precision_path,'{}.txt'.format(tracker_name+video.name))
                with open(precision_result_path, 'w') as f:
                    for x in precision_ret_[video.name]:
                        f.write(str(x) + '\n')
